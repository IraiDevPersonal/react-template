...
export default function App() {
  ...
  return (
    <div className='container mx-auto h-screen w-full grid place-content-center gap-3'>
      ...
      <Input startAdorment={<>https://</>} endAdorment={<IconBackspace />} label='test' value={value} onChange={(e) => { setValue(e.target.value) }} helperText={value} />
      ...
      <button className={`py-2 px-4 transition-colors duration-300 font-semibold ${color({ variant: 'smooth', color: 'error' })}`}>
        boton
      </button>
    </div>
  )
